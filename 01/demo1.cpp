...
bool CylinderSegment(PointCloud::Ptr& cloud, vector<vector<Point3d>>& axis)
{
	...
	do
	{
		...
		if (inners.size() > minPointNum)
		{
			...
			pcl::io::savePCDFileBinary("./" + pngnum + "_cylinder_" + std::to_string(iters) + ".pcd", *cylinder_segment);
			Eigen::VectorXf coef;
...
int main(int argc, char** argv)
{
	...
	cv::Mat depth;
	string image_path = "./" + pngnum + ".png";            //待处理图路径	
	...
	doube Distance;
	CalDistance(axis[0][i], axis[1][i], axis[0][j],axis[1][j]);
